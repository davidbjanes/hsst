...
for rep = 1:opts.reps                                 % TOTAL # REPETITIONS
	...
	for iter = 1:opts.divisions                       % # K-MEANS SPLITS
        ...
        if ((iter==opts.divisions) && debug)  % do some extra splitting on clusters that look too big
            ...
            splitbig = centroid(toobig,:) + randn(rand_stream,length(toobig),size(centroid,2));
            centroid = [centroid; splitbig];
        end
        
        centroid = [centroid; centroid] + jitter * randn(rand_stream,2*size(centroid, 1), size(centroid,2)); % split & jitter
...
cmap = jetm(numclusts);
spikes.info.kmeans.colors = cmap(randperm(rand_stream,numclusts),:);
